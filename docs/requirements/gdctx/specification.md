...
- использовать `gdgraph` для выбора релевантных файлов, когда граф доступен;
...
Возвращает компактный статус проекта: Git branch, dirty files grouped by status, наличие `.metaproject`, включенные модули, stale/missing gdgraph artifacts и последние gdctx artifacts.
...
- если доступен `gdgraph`, добавляет affected hints для измененных файлов.
...
- если pattern похож на имя символа/модуля, сначала пробует `gdgraph` для narrowing.
...
Для TypeScript/JavaScript файлов `outline` должен использовать AST, когда это возможно.
...
Каждый запуск должен создавать metadata:

```json
{
  "id": "2026-07-06T10-15-30Z_ctx_diff",
  "module": "gdctx",
  "command": "git diff",
  "exitCode": 0,
  "rawPath": ".metaproject/data/gdctx/raw/2026-07-06T10-15-30Z_ctx_diff.log",
  "summaryPath": ".metaproject/data/gdctx/artifacts/latest.md",
  "bytesIn": 184221,
  "bytesOut": 9210,
  "truncated": true,
  "graphContext": {
    "used": true,
    "commands": ["gd-metapro gdgraph affected src/foo.ts"]
  }
}
```
