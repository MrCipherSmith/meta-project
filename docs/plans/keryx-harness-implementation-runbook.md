...
| **R2-5** | **real-subprocess executor (закрывает F-1 / SC_R04 live)** | SC_R04_SHELL_CONTAINMENT (runtime half) | ✅ | `feature/keryx-release2-subprocess-executor` | 2026-07-13 | flow 026 (option B, PR в main). **Закрывает F-1** — рантайм-половину `SC_R04_SHELL_CONTAINMENT` (W10 построил структурную: allowlist/fingerprint/injection-deny/approval/fail-closed-isolation `8ed5373`). Новый `src/harness/process/` — **`runContainedProcess(input, deps)`** (композиция W10 `guardAction`/`actionFingerprint`/`ExecutionReceipt` + W12 `inheritBudget` + W7 evidence). Порт **`ProcessAdapter`** (единственная side-effect-граница) + типизированный исход `completed`\|`timeout`\|`output-overflow`\|`cancelled`\|`blocked`. Gate-order fail-closed (adapter НЕ спавнится при deny): (1) argv/env allowlist (REUSED `guardAction`+`actionFingerprint`; любой env-ключ вне allowlist → blocked) → (2) budget (REUSED `inheritBudget`, breach → blocked) → (3) `adapter.spawn` РОВНО один раз в approved cwd → (4) классификация observation: любой bound-hit (timeout/overflow/cancel/spawn-error) — записанный **NON-success** (никогда `completed`), с receipt/evidence. **Split (W14-прецедент, предодобрен):** оффлайн-сьют = только детерминированный fake-адаптер; тонкий реальный `node:child_process`-адаптер (`real-process-adapter.ts`) за capability-флагом `allowRealSubprocess`/`KERYX_ALLOW_REAL_SUBPROCESS=1`, **НЕ в CI**. **Live-smoke прогнан локально (darwin):** `/bin/echo`→`clean-exit`, `sleep 30`@500ms→`deadline-exceeded`. **Review (highload/security): PASS** — no fail-open, unapproved argv/env НЕ достигает spawn, bound-hit никогда не `completed`, D-02, secrets (env-values) не логируются. **Live-run вскрыл реальный zombie-баг** в первичном async+detached+poll (exited detached child → зомби → `kill(pid,0)` вечно alive → clean-exit не наблюдался): переписан на `spawnSync` (единственный sync-примитив, который RUN+REAP+читает реальный exit/output+timeout). **Disclosed design-constraint:** sync-порт → deadline-kill leader-only (прямой child убит+reap; group-kill внуков — async-follow-up); оффлайн-ядро моделирует полный process-group no-orphan контракт через fake. AC1–AC5 ✅. `tsc` clean, `bun test` **1273/0** (+2 skip flag-smoke; 1254+18 оффлайн+3 smoke); deps `{}` (child_process — stdlib); reuse-only (W10/W12/W7/W8 — 0 правок, `git diff` пуст); D-02 (не пишет flow.json); frozen/канонические схемы/src/eval/src/contracts/ADR не тронуты; без соавторства. Модели: impl+review — Opus, тесты — Sonnet. **🎯 Release 2 ЗАВЕРШЁН (R2-1…R2-5).** |
| **R2-postreview** | **харденинг по полному ревью новой функциональности** | 5 MED review-findings | ✅ | `feature/keryx-r2-review-hardening` | 2026-07-14 | flow 027 (option B, PR в main). Полное 5-линзовое ревью R2-поверхности (security / highload+logic / architecture / clean-code / testing) → **0 BLOCKER/HIGH**, 5 MED закрыты TDD: **#1** SSRF-гард в `select.ts` `probeOllamaModels` (reuse `isPrivateEgressHost`/`isLoopbackHost` до fetch — private/metadata хост → ollama omitted, 0 fetch; loopback пробится); **#2** `executor.ts` — `exitCode` пробрасывается в исход `completed` (ненулевой exit остаётся `completed`/контейнмент, но код восстановим); bound-hits без изменений; **#3** `real-process-adapter.ts` — чистый `classifyProcessResult` (crash-сигнал SIGSEGV/SIGTERM ≠ `deadline-exceeded`; только ETIMEDOUT/SIGKILL+deadlineHit → deadline; unit-тест офлайн без spawn); **#4** два mutation-proof теста executor (cancel-поверх-clean-exit; finer output-limit reclassification); **#5** dedup `buildExt` в `execute.ts` (поведение неизменно). **Review (security): PASS** — SSRF закрыт до fetch, нет нового fail-open, crash никогда не ложный `completed`/`deadline`, reuse-only (guard.ts не тронут). AC1–AC5 ✅. `tsc` clean, `bun test` **1323/0** (+2 skip; 1306+новые); live-smoke 3/3 (после #3-рефактора). deps `{}`; D-02; secrets не логируются; frozen не тронут; без соавторства. Модели: impl+review — Opus, тесты — Sonnet. LOW/INFO из ревью — отложены. |
